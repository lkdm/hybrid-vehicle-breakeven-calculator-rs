...
#[component]
pub fn NumberInput(handle_input: WriteSignal<String>, value: ReadSignal<String>) -> impl IntoView {
    ...
    view! {
        <input
            ...
            on:input=move |ev| {
                handle_input.set(event_target_value(&ev))
            }
            prop:value=value
        />
    }
...
fn FormExample() -> impl IntoView {
    ...
    let (fuel_cost, set_fuel_cost) = signal(String::new());
    let (hybrid_upfront_cost, set_hybrid_upfront_cost) = signal(String::new());
    let (hybrid_efficiency, set_hybrid_efficiency) = signal(String::new());
    let (petrol_upfront_cost, set_petrol_upfront_cost) = signal(String::new());
    let (petrol_efficiency, set_petrol_efficiency) = signal(String::new());
    let (annual_km_driven, set_annual_km_driven) = signal(String::new());

    let (hybrid_fuel_cost, set_hybrid_fuel_cost) = signal(String::new());
    let (petrol_fuel_cost, set_petrol_fuel_cost) = signal(String::new());
    let (upfront_cost_difference, set_upfront_cost_difference) = signal(String::new());
    let (breakeven_point, set_breakeven_point) = signal(String::new());
    let (per_kilometre_fuel_cost_difference, set_per_kilometre_fuel_cost_difference) =
        signal(String::new());
    let (breakeven_point_years, set_breakeven_point_years) = signal(String::new());

    Effect::new(move |_| {
        let _fuel_cost: f64 = fuel_cost.get().parse().ok().unwrap_or(0.0);
        // logging::log!("Fuel cost: {}", _fuel_cost);
        let _hybrid_upfront_cost: f64 = hybrid_upfront_cost.get().parse().ok().unwrap_or(0.0);
        // logging::log!("Hybrid upfront cost: {}", _hybrid_upfront_cost);

        let _hybrid_litres_per_distance_unit: f64 =
            hybrid_efficiency.get().parse().ok().unwrap_or(0.0);
        // logging::log!("Hybrid efficiency: {}", _hybrid_litres_per_distance_unit);

        let _petrol_upfront_cost: f64 = petrol_upfront_cost.get().parse().ok().unwrap_or(0.0);
        // logging::log!("Petrol upfront cost: {}", _petrol_upfront_cost);

        let _petrol_litres_per_distance_unit: f64 =
            petrol_efficiency.get().parse().ok().unwrap_or(0.0);
        ...
        let _annual_km_driven: f64 = annual_km_driven.get().parse().ok().unwrap_or(0.0);
